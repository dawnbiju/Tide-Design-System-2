...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="flex flex-col gap-6">
      <h2 className="font-display font-semibold text-heading-l leading-10 tracking-[0.5px] text-[var(--color-text-primary)] border-b border-[var(--color-border-subtle)] pb-4">
        {title}
      </h2>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg-page)]">
      {/* ── Page Banner ── */}
      <Banner variant="info" title="Design System Preview" description="This is the live Tide Design System component showcase." dismissible />

      {/* ── Hero ── */}
      <div className="bg-[var(--color-bg-brand-strong)] px-16 py-24 flex flex-col gap-6">
        <p className="font-body text-body-m tracking-[0.25px] text-[var(--color-text-inverse)] opacity-70 uppercase">Tide Design System 2</p>
        <h1 className="font-display font-semibold text-display-xl leading-[68px] tracking-[0.5px] text-[var(--color-text-inverse)] max-w-3xl">
        ...
        <div className="flex gap-4 mt-4">
          <Button variant="primary" theme="inverse" label="Get started" />
          <Button variant="secondary" theme="inverse" label="View on GitHub" />
        </div>
      </div>

      {/* ── Component Gallery ── */}
      <main className="max-w-[1280px] mx-auto px-16 py-24 flex flex-col gap-24">
        ...
        <Section title="Button">
          ...
          <div className="flex gap-4 bg-[var(--color-bg-brand-strong)] p-4 rounded-[var(--radius-16)]">
        ...
        <Section title="Link">
          ...
          <div className="flex gap-4 bg-[var(--color-bg-brand-strong)] px-6 py-4 rounded-[var(--radius-16)]">
        ...
        <Section title="Feature Card">
          <FeatureCard theme="light"   title="Recognition budget"   body="Each team gets room to recognise people in personal, meaningful ways." className="max-w-sm" />
          <FeatureCard theme="brand"   title="Recognition budget"   body="Each team gets room to recognise people in personal, meaningful ways." className="max-w-sm" />
          <FeatureCard theme="inverse" title="Recognition budget"   body="Each team gets room to recognise people in personal, meaningful ways." className="max-w-sm" />
        </Section>
        ...
        <Section title="Text Input">
          ...
          <div className="w-80">
            <TextInput label="Amount" affix="prefix" placeholder="0.00" helper="Enter your transfer amount." state="default" />
          </div>
          <div className="w-80">
            <TextInput label="Username" placeholder="@handle" affix="suffix" suffixSymbol="#" state="error" helper="Username already taken." />
          </div>
          <div className="w-80">
        ...
        <Section title="Card">
          {(['standard', 'elevated', 'outlined', 'interactive', 'expandable'] as const).map(v => (
            <Card
              ...
              expandedContent="Additional hidden details appear here when expanded."
              className="w-72"
            />
          ))}
        </Section>
        ...
        {/* Alerts */}
        <Section title="Alert / Toast / Banner">
        ...
        <Section title="Navigation">
          <div className="flex flex-col gap-6 w-full">
            <div>
              <p className="font-body font-semibold text-body-s text-[var(--color-text-secondary)] mb-3 uppercase tracking-widest">Tabs</p>
              <TabBar
                items={[
            ...
            <div className="flex gap-12">
              <div>
                <p className="font-body font-semibold text-body-s text-[var(--color-text-secondary)] mb-3 uppercase tracking-widest">Sidebar</p>
                <Sidebar
                  items={[
              ...
              <div>
                <p className="font-body font-semibold text-body-s text-[var(--color-text-secondary)] mb-3 uppercase tracking-widest">Breadcrumbs</p>
                <Breadcrumbs
                  items={[
      ...
      {/* Footer */}
      <footer className="bg-[var(--color-bg-brand-strong)] px-16 py-12 flex items-center justify-between">
        <p className="font-display font-semibold text-heading-s leading-8 tracking-[0.25px] text-[var(--color-text-inverse)]">
